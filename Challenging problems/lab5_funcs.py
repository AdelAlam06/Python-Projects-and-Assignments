...
def mixedfraction(num, den):
    ...
    numerator = num - den * wholeNum

    return wholeNum, numerator, den
...
def iscyclops(n):
    ...
    '''
    stringOfNum = str(n)
    quotient = len(stringOfNum)//2
    if (stringOfNum[quotient] == "0" and len(stringOfNum) % 2 != 0
            and stringOfNum.count("0") == 1):
        return True
    return False
...
def altsignsum(items):
    ...
    sumVal = 0
    if (items == False):  # empty lists are False by default
        return 0
    elif (len(items) == 1):
        return items[0]
...
def domninocycle(tiles):
    ...
    for i in range(len(tiles)):
        ...
        if (i == len(tiles)-1):
            if (tiles[0][0] == tiles[i][1]):  # checks for first first and last last value
                continue  # if they match then its a loop otherwise False
            else:
                return False

        elif (tiles[i][1] == tiles[i+1][0]):
            continue
        else:
            return False
