...
def stringtofloatlist(fltstr):
    ...
    '''
    floatList = []
    listFloat = fltstr.split(",")
    for i in range(len(listFloat)):

        floatList.append(float(listFloat[i]))
    return floatList
...
def maxbytype(items):
    ...
    return intTemp, fltTemp, strTemp
